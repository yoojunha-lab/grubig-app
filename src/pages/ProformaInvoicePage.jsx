...
export const ProformaInvoicePage = ({
  ...
  const handlePickFabric = (fabricId) => {
    ...
    setFabricPick(''); // 선택 후 초기화 (연속 추가 편의)
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 w-full print:hidden">
      {/* 헤더 + 툴바 */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
          <button onClick={() => handleNewPI(piInput.marketType)} className="bg-white border border-slate-300 text-slate-600 px-3 py-2 rounded-lg hover:bg-slate-50 hover:border-slate-400 flex items-center justify-center gap-1.5 text-sm font-medium transition-colors">
            <FilePlus className="w-4 h-4" /> 새 문서
          </button>
          <button onClick={handleSavePI} className="bg-slate-900 text-white px-4 py-2 rounded-lg hover:bg-slate-800 flex items-center justify-center gap-1.5 text-sm font-bold">
            <Save className="w-4 h-4" /> 저장
          </button>
          <button onClick={() => handlePrintPI(piInput)} className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 flex items-center justify-center gap-1.5 text-sm font-bold shadow-lg shadow-indigo-200">
            <Download className="w-4 h-4" /> PDF 인쇄
          </button>
          <button onClick={() => handleDownloadPIExcel(piInput)} className="bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 flex items-center justify-center gap-1.5 text-sm font-bold shadow-lg shadow-emerald-200">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {/* 수출/내수 토글 + 편집중 표시 */}
        <div className="flex items-center justify-between bg-slate-50 border-b border-slate-200 px-4 sm:px-6 py-3">
          <div className="flex bg-white p-1 rounded-xl border border-slate-200">
            <button
              onClick={() => setMarketType('export')}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-bold transition-all ${isExport ? 'bg-gradient-to-r from-emerald-600 to-emerald-500 text-white shadow' : 'text-slate-500 hover:text-slate-700'}`}
            >
              <Globe className="w-4 h-4" /> 수출용 PI
            </button>
            <button
              onClick={() => setMarketType('domestic')}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-bold transition-all ${!isExport ? 'bg-gradient-to-r from-blue-600 to-blue-500 text-white shadow' : 'text-slate-500 hover:text-slate-700'}`}
            >
          ...
          <div className="text-xs font-bold">
            {editingPIId
              ? <span className="text-indigo-600 flex items-center gap-1"><Pencil className="w-3.5 h-3.5" /> 편집 중: {piInput.piNo}</span>
              : <span className="text-slate-400">새 문서 작성 중</span>}
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div>
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => handleNewPI(piInput.marketType)} title="번호 자동 채번(새 문서)" className="shrink-0 px-2 border border-slate-200 rounded-lg text-slate-400 hover:text-indigo-600 hover:border-indigo-300">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <h3 className="text-lg font-bold text-slate-800">보관함 <span className="text-sm text-slate-400">({filteredList.length})</span></h3>
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              <input value={listSearch} onChange={e => setListSearch(e.target.value)} placeholder="번호·바이어 검색"
                className="border border-slate-200 rounded-lg pl-8 pr-3 py-2 text-sm w-48 focus:ring-2 focus:ring-indigo-500 outline-none" />
            </div>
          </div>
        </div>

        {filteredList.length === 0 ? (
          <div className="py-12 text-center text-slate-400 text-sm">저장된 문서가 없습니다.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[720px]">
              <thead>
                <tr className="text-left text-[11px] text-slate-400 uppercase border-b border-slate-200">
                  <th className="py-2 px-2 font-bold">구분</th>
                  <th className="py-2 px-2 font-bold">문서번호</th>
                  <th className="py-2 px-2 font-bold">발행일</th>
                  <th className="py-2 px-2 font-bold">바이어</th>
                  <th className="py-2 px-2 font-bold text-right">합계</th>
                  <th className="py-2 px-2 font-bold text-center">작업</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filteredList.map(pi => {
                  const t = computePITotals(pi);
                  const sym = (pi.marketType === 'domestic') ? '₩' : '$';
                  const amt = (pi.marketType === 'domestic')
                  ...
                  return (
                    <tr key={pi.id} className={`hover:bg-slate-50 ${editingPIId === pi.id ? 'bg-indigo-50/40' : ''}`}>
                      <td className="py-2.5 px-2">{marketBadge(pi.marketType)}</td>
                      <td className="py-2.5 px-2 font-bold text-slate-800">{pi.piNo}</td>
                      <td className="py-2.5 px-2 text-slate-500">{pi.date}</td>
                      <td className="py-2.5 px-2 text-slate-700 uppercase">{pi.buyerCompany}</td>
                      <td className="py-2.5 px-2 text-right font-mono text-slate-800">{sym}{amt}</td>
                      <td className="py-2.5 px-2">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => handleEditPI(pi.id)} title="편집" className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded"><Pencil className="w-4 h-4" /></button>
                          ...
                          <button onClick={() => handleDuplicatePI(pi.id)} title="복제" className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded"><Copy className="w-4 h-4" /></button>
