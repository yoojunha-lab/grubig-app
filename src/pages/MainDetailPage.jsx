  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        {/* 좌측: 신규/수정 폼 */}
        <div className="xl:col-span-4 bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
            ...
            <div className="flex gap-2">
              {editingDetailId && (
                <button onClick={resetDetailForm} className="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded">취소</button>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-1">
                <label className="block text-xs font-bold text-slate-500 mb-1">Article No *</label>
            ...
            {/* 저장 버튼 */}
            <div className="pt-2">
               <button onClick={handleSaveDetail} className="w-full bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-bold py-3 rounded-xl shadow-lg shadow-fuchsia-600/30 active:scale-95 transition-all flex items-center justify-center gap-2">
        ...
        {/* 우측: 데이터 리스트 */}
        <div className="xl:col-span-8 bg-white p-5 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden">
            ...
            <table className="w-full min-w-[800px] border-collapse">
              <thead>
                ...
                <tr className="bg-slate-100 border-b-2 border-slate-300">
                  ...
                  <th className="p-1 px-2 text-[10px] text-slate-500 font-normal text-left border-r border-slate-200">Article / Order / Color / LOT</th>
                    ...
                    <tr key={d.id} className="hover:bg-slate-50 transition-colors">
                      <td className="p-2 border-r border-slate-200">
                        <div className="flex gap-1 justify-center">
                          <button onClick={() => handleEditDetail(d.id)} className="p-1 text-slate-400 hover:text-fuchsia-600 bg-slate-100 rounded hover:bg-fuchsia-50 transition-colors" title="수정"><Edit2 className="w-3.5 h-3.5"/></button>
                      ...
                      <td className="p-2 border-r border-slate-200">
                        <div className="font-mono text-sm font-bold text-fuchsia-700">{d.articleNo}</div>
                        <div className="text-[10px] text-slate-500 flex gap-1 flex-wrap mt-0.5">
                          {d.orderNo && <span className="bg-slate-100 px-1 rounded border border-slate-200">O: {d.orderNo}</span>}
                          {d.colorInfo && <span className="bg-slate-100 px-1 rounded border border-slate-200">C: {d.colorInfo}</span>}
                          {d.lotNo && <span className="bg-slate-100 px-1 rounded border border-slate-200">L: {d.lotNo}</span>}
                        </div>
                      </td>
                      ...
                      <td className="p-2 text-center text-[10px] font-bold h-full align-middle">
                        {latestTest ? (
                          isPass ? <span className="flex flex-col items-center justify-center text-emerald-600 bg-emerald-50 py-1 rounded w-full border border-emerald-100"><CheckCircle2 className="w-3.5 h-3.5 mb-0.5"/> <span>PASS <span className="text-[8px] font-normal">({d.tests.length}차)</span></span></span> 
                                 : isFail ? <span className="flex flex-col items-center justify-center text-red-600 bg-red-50 py-1 rounded w-full border border-red-100"><XCircle className="w-3.5 h-3.5 mb-0.5"/> <span>FAIL <span className="text-[8px] font-normal">({d.tests.length}차)</span></span></span> 
                                          : <span className="text-slate-400">판정대기</span>
